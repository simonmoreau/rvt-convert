...
import { ForgeUpload, IUploadObject } from './forge-upload.class';
...
export class FileUploader {
  ...
  public options: FileUploaderOptions = {
    ...
    formatDataFunction: (item: FileItem) => item.file,
    formatDataFunctionIsAsync: false
  };

  protected _failFilterIndex: number;

  public constructor(options: FileUploaderOptions, private http: HttpClient) {
  ...
  protected _xhrTransport(item: FileItem): any {
    ...
    if (!this.options.disableMultipart) {
      ...
      const appendFile = () => sendable.append(item.alias, item.file, item.fileLikeObject.name);
      ...
      if (this.options.additionalParameter !== undefined) {
        Object.keys(this.options.additionalParameter).forEach((key: string) => {
          ...
          if (typeof paramVal === 'string' && paramVal.indexOf('{{file_name}}') >= 0) {
            paramVal = paramVal.replace('{{file_name}}', item.fileLikeObject.name);
          }
