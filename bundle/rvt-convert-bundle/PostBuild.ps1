...
if (Test-Path $addinFolder) {
    try {
        ...
        # create the RVTConvertBundle folder
...
xcopy /Y ($ProjectDir + $TargetName + ".addin") ($ProjectDir + "\RVTConvertBundle.bundle\Contents")
xcopy /Y ($TargetDir + "\*.dll*") ($ProjectDir + "\RVTConvertBundle.bundle\Contents\RVTConvertBundle")

# Zip the folder

$ReleasePath="C:\Users\Simon\Github\Forge\rvt-convert\deploy"
$ReleaseZip = ($ReleasePath + "\" + $TargetName + ".zip")

if ( Test-Path -Path $ReleasePath ) {
  7z a -tzip $ReleaseZip ($ProjectDir + "\RVTConvertBundle.bundle")
}
