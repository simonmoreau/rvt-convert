...
namespace RVTConvertBundle
{

  [Autodesk.Revit.Attributes.Regeneration(Autodesk.Revit.Attributes.RegenerationOption.Manual)]
  [Autodesk.Revit.Attributes.Transaction(Autodesk.Revit.Attributes.TransactionMode.Manual)]
  public class RVTConvertBundleApp : IExternalDBApplication
  {
    public ExternalDBApplicationResult OnStartup(Autodesk.Revit.ApplicationServices.ControlledApplication app)
    {
      ...
      RVTConvertBundleApp.LogTrace(string.Format("Running Version {0} - {1}", version.ToString(), buildDate.ToString()));


      // Hook up the CustomFailureHandling failure processor.
      RVTConvertFailuresProcessor rvtConvertFailuresProcessor = new RVTConvertFailuresProcessor();
      app.FailuresProcessing += rvtConvertFailuresProcessor.FailureProcessor;
      RVTConvertBundleApp.LogTrace("Custom Failures Processor registered.");
    ...
    {
      ExportRevitFile(e.DesignAutomationData);

      e.Succeeded = true;
    }

    private void ExportRevitFile(DesignAutomationData data)
    {
      ...
      if (inputDoc.IsFamilyDocument)
      {
        ...
        RVTConvertBundleApp.LogTrace("Loaded Family In Document.");
      }
      else
      {
        projectDocument = inputDoc;
        RVTConvertBundleApp.LogTrace("Loaded Project Document.");
      }
    ...
    private void ExportDocumentToIFC(Document projectDocument)
    {
      using (Transaction transaction = new Transaction(projectDocument))
      {
        ...
        RVTConvertBundleApp.LogTrace("Start IFC conversion.");
